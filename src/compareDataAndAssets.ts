...
export async function compareDataAndAssets() {
  ...
  const registeredAmount = (
    ...
    (previousValue, currentValue) => {
      return {
        ...previousValue,
        [currentValue.brand]: String(currentValue.now_amount.toDP(ASSETS_DECIMAL_PLACES))
      };
    },
    {} as { [key: string]: string }
  );
  ...
  for (let key in assetsData) {
    ...
    if (
      ...
      Math.abs(Number(assetsData[key]) - Number(registeredAmount[key])) > ASSETS_COMPARISON_TOLERANCE
    ) {
      message = `'${key}' is wrong\tnow:${assetsData[key]},\tDB:${registeredAmount[key]}`;
      result.ng.push(message);
    } else if (key in registeredAmount && assetsData[key] === registeredAmount[key]) {
