...
const defaultOptions: SymmetricalTriangleOptions = {
  minBars: 36,
 ...
 * - 上ラインは負傾き、下ラインは正傾き
 * - 幅が十分に収束（開始時より終端で狭い）
 * - 最新値が上ラインを上抜け（直前バーは未突破）
 */
export function detectSymmetricalTriangleBreakout(
  bars: PriceBar[],
  options: Partial<SymmetricalTriangleOptions> = {}
): boolean {
...
function evaluateSymmetricalTriangleBreakout({ bars }: ChartPatternEvaluateInput): boolean {
  return detectSymmetricalTriangleBreakout(bars);
}

export const symmetricalTriangleBreakoutDetector: ChartPatternDetector = {
  id: 'symmetrical_triangle_breakout',
  titleJa: 'A - 三角持ち合い上抜け',
  signal: 'buy',
  evaluate: evaluateSymmetricalTriangleBreakout
};
