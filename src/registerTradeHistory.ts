...
export const registerDataByLambda = async (data: any[], serviceFlag?: ServiceFlag): Promise<number> => {
  if (serviceFlag === 'GMO') {
    // GMOデータの場合23列のため
    ...
    await prisma.$transaction(
      async (prisma) => {
        for (let i = 0; i < data.length; i++) {
          ...
          if (latestRegisteredDate && new Date(trade_date) <= latestRegisteredDate) {
...
if (process.argv[1] === __filename) {
  ...
  if (process.argv.length !== 3) {
    console.error('Error: Usage: npx ts-node src/registerTradeHistory.ts (Input file name)');
    process.exit(1);
  }
  ...
  (async function registerData() {
    try {
      const data: string = await readFile(path.resolve(__dirname, inputFilePath), 'utf8');
      let passed = 0;
      ...
      await prisma.$transaction(
        async (prisma) => {
          for (let i = 0; i < line.length; i++) {
            ...
            const item = l.split(',');
            if (latestRegisteredDate && new Date(item[0]) <= latestRegisteredDate) {
              // DBにある最新の日時よりも前 -> すでに登録済みとみなし、スキップ
              passed++;
              continue;
            }
